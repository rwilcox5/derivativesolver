...
def checkpower(inputexpression,dvar,idvar,wrongness):
	...
	if inputexpression[0:5]=='sqrt(':
		if fullparen(inputexpression[4:]):
			return [True,powerrulew(inputexpression,inputexpression[4:],idvar,wrongness)[1],['sqrt',inputexpression[4:]],'Apply the chain rule to <div class="katex_div_il">f(g('+dvar+'))</div> with <div class="katex_div_il">f(U)=sqrt(U)</div> and <div class="katex_div_il">g('+dvar+')='+slatex(inputexpression[4:])+'</div>']
